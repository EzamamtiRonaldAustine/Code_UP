...
\begin{titlepage}
  ...
  \includegraphics[width=0.8\textwidth]{img/UCU.png}\\[1.5cm]

  % Main project title and course
  {\color{PrimaryBlue}\Huge\bfseries Yahboom G1 Tank Robot}\\[0.4em]
  {\color{MidBlue}\Large Laboratory Tasks}\\[0.6em]
  {\large Advanced Robotics \& Control Systems}\\[2cm]

  % Author block
  {\large By}\\[0.3cm]
  {\Large\bfseries EZAMAMTI RONALD AUSTINE}\\[0.4cm]
  {\large Registration No.: S23B23-018}\\[0.2cm]
  {\large Access No.: B24252}\\[2cm]
  ...
  \centering\small
  \textbf{Hardware Constraint Note:} The Yahboom Raspberry Pi OS image failed to boot
  during the lab session. The Arduino UNO configuration was used instead. Camera
  pan and tilt servos (DOF 4 \& 5) are wired only to the Pi GPIO and were not
  physically tested; their results are therefore \textbf{THEORETICAL}.
  \end{tcolorbox}
...
\begin{tabularx}{\linewidth}{L{3.5cm} L{2.8cm} C{3cm} X}
\toprule
\rowcolor{MidBlue}
...
\begin{donebox}{How to Physically Verify POE Results}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{C{1.8cm} C{2cm} C{2cm} L{4cm}}
\toprule
\rowcolor{MidBlue}
...
\midrule
$0^\circ$   & 500  & $0$\,m   & Faces $90^\circ$ right \\
$45^\circ$  & 995  & $+0.057$\,m & $45^\circ$ right, tip $5.7$\,cm left \\
$90^\circ$  & 1490 & $+0.080$\,m & Faces forward, tip at origin \\
$135^\circ$ & 1985 & $+0.057$\,m & $45^\circ$ left \\
$180^\circ$ & 2480 & $0$\,m   & Faces $90^\circ$ left \\
\bottomrule
\end{tabular}
\end{table}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{L{2.5cm} C{2.5cm} C{2.5cm} C{2.5cm} L{3cm}}
\toprule
\rowcolor{MidBlue}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{L{2.5cm} C{1.5cm} C{1.8cm} C{2cm} L{3.5cm}}
\toprule
\rowcolor{MidBlue}
...
\midrule
Initial (Home) & $90^\circ$ & 1490 & Green & Sensor faces forward \\
Position 1 & $45^\circ$ & 995 & Blue & $45^\circ$ right of fwd \\
Position 2 (Min) & $0^\circ$ & 500 & Red & Full right limit \\
Position 3 & $135^\circ$ & 1985 & Blue & $45^\circ$ left of fwd \\
Position 4 (Max) & $180^\circ$ & 2480 & Red & Full left limit \\
Final (Return) & $90^\circ$ & 1490 & Green & Returns to forward \\
\bottomrule
\end{tabular}
\end{table}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{L{2.5cm} C{1.5cm} C{1.8cm} L{3.5cm} C{2.8cm}}
\toprule
\rowcolor{MidBlue}
\color{white}\textbf{Position} &
\color{white}\textbf{Angle} &
