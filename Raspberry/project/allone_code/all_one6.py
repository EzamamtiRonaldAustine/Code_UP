    ...
    def _update_lcd_content(self, status):
        ...
        try:
            ...
            if status['overall'] in ['WARNING', 'CRITICAL']:
                content = f"* {status['overall']} *\n{status['alerts'][0][:16]}" if status['alerts'] else f"* {status['overall']} *"
            else:
        ...
        for attempt in range(max_retries):
            ...
            if time.time() - self.state['thingspeak']['last_network_check'] > 30:
                ...
                if not check_network_connectivity():
                    ...
                    # Try network restart every 3 failures
            ...
            try:
                ...
                if response.status_code == 200:
                    result = response.text.strip()
                    if result.isdigit() and int(result) > 0:
                        logger.info(f"✅ ThingSpeak upload successful (Entry: {result})")
            ...
            # CRITICAL FIX: Use random.uniform instead of time.uniform
            if attempt < max_retries - 1:
