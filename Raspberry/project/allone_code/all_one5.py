...
DEFAULT_CONFIG = {
    ...
    "THINGSPEAK": {
        ...
        # IMPORTANT: Verify this API key in your 'pond_config.json' file.
        # The log showed a different key than the default.
...
def check_network_connectivity():
    """Check if the device has internet connectivity with a fallback."""
    try:
...
def restart_network_interface():
    ...
    try:
        ...
        # This command might need adjustment based on your OS (e.g., 'NetworkManager.service')
        subprocess.run(["sudo", "systemctl", "restart", "networking"], check=True)
        time.sleep(10)  # Wait for network to come back up
        return True
...
class SmartFishPondMonitor:
    def __init__(self):
        ...
        self.state = {
            ...
            'indicators': {
                ...
                'last_lcd_content': "",
                'lcd_error_count': 0,
                'lcd_available': True  # Track LCD availability
            },
            'sms': {
                'last_sent_time': 0
            },
    ...
    def _init_lcd(self):
        ...
        # Mark LCD as unavailable after all retries fail
        ...
        for param, (min_val, max_val) in valid_ranges.items():
            ...
            try:
                ...
                if not (min_val <= val_float <= max_val):
                    ...
                    # Cap phosphorus and nitrogen at max_range instead of rejecting
    ...
    def send_to_thingspeak(self, data, retry_count=0):
        """Send sensor data to ThingSpeak with retry logic. Returns True on success."""
        if not config['THINGSPEAK']['API_KEY']:
            logger.warning("ThingSpeak API key not configured")
            return False

        current_time = time.time()
        if current_time - self.state['thingspeak']['last_network_check'] > 60:
            self.state['thingspeak']['last_network_check'] = current_time
            if not check_network_connectivity():
                logger.warning("No network connectivity. Attempting to restart network interface.")
                if restart_network_interface():
                    logger.info("Network interface restarted successfully")
                else:
                    logger.error("Failed to restart network interface")
                    return False
        ...
        try:
            ...
            if retry_count < config['THINGSPEAK']['MAX_RETRIES']:
                delay = min(
                    config['THINGSPEAK']['RETRY_DELAY'] * (2 ** retry_count),
                    config['THINGSPEAK']['MAX_RETRY_DELAY']
                )
                logger.info(f"Retrying ThingSpeak connection in {delay} seconds (attempt {retry_count + 1}/{config['THINGSPEAK']['MAX_RETRIES']})")
                time.sleep(delay)
                return self.send_to_thingspeak(data, retry_count + 1)
            else:
                logger.error(f"Max retries ({config['THINGSPEAK']['MAX_RETRIES']}) exceeded. Giving up.")
                return False
        except Exception as e:
    ...
    def flush_thingspeak_backup(self):
        ...
        backup_file = config['THINGSPEAK']['BACKUP_FILE']
        if not os.path.exists(backup_file):
            # If the file doesn't exist, there's nothing to flush.
            return
                ...
                with self.state_lock:
                    if time.time() - self.state['thingspeak']['last_sent_time'] >= config['THINGSPEAK']['SEND_INTERVAL']:
                        ...
                        if self.has_any_valid_data(data):
                            # Safely attempt to flush any backed-up data first
                            try:
                                self.flush_thingspeak_backup()
                            except Exception as e:
                                logger.error(f"Critical error flushing backup: {e}")
                                # Ensure backup file exists for future saves
            ...
            try:
                ...
                for thread_name, last_update in self.thread_watchdog.items():
                    if current_time - last_update > timeout:
                        ...
                        self.thread_watchdog[thread_name] = current_time # Reset to prevent spamming
...
def main():
    logger.info("Smart Fish Pond Monitoring System v5.11 (Robust)")
    
    # Ensure the backup file exists before starting the monitor
    ensure_backup_file_exists()
