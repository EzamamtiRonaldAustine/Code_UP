...
SMS_COOLDOWN = 300  # seconds (5 minutes)
CRITICAL_DURATION = 300 # seconds (5 minutes)
HISTORY_SIZE = 5  # number of readings to keep for trend analysis

# --- ThingSpeak Configuration ---
THINGSPEAK_URL = "https://api.thingspeak.com/update"
THINGSPEAK_API_KEY = "A1NXZG51WFRS96Y7"  # <-- REPLACE with your Write API Key
THINGSPEAK_SEND_INTERVAL = 60  # seconds between ThingSpeak updates (min 15 for free tier)
THINGSPEAK_BACKUP_FILE = "/home/pi/pond_thingspeak_backup.csv"
...
class SmartFishPondMonitor:
    def __init__(self):
        ...
        self.thingspeak_thread = threading.Thread(target=self.thingspeak_loop)

        for t in [self.monitor_thread, self.display_thread, self.pump_thread, self.thingspeak_thread]:
            t.daemon = True
            t.start()
    ...
    def get_water_quality_status(self, data, ds18b20_temp):
        ...
        # Scoring and alert logic (same as original)
        if avg_ph is None:
            ...
            if value is not None:
                try:
                    ...
                    if not (min_val <= val_float <= max_val):
                        print(f"  ⚠️  {param} out of range: {val_float} (expected {min_val}-{max_val})")
                        return False
                except (ValueError, TypeError):
                    print(f"  ⚠️  {param} is not a valid number: {value}")
                    return False
        return True
    ...
    def send_to_thingspeak(self, data):
        ...
        if not THINGSPEAK_API_KEY:
            print("  ⚠️  ThingSpeak API key not configured")
            return False
        
        # Validate data before sending
        if not self.validate_sensor_data(data):
            print("  ⚠️  Data validation failed. Skipping send.")
            return False
        ...
        payload = {
            ...
            "field2": data.get('ph'),
            "field3": data.get('ec'),
            "field4": data.get('nitrogen'),
            "field5": data.get('phosphorus'),
            "field6": 1 if data.get('turbidity') else 0,
            "field7": data.get('quality_score'),
            "field8": data.get('quality_status')
        }
        
        try:
            r = requests.get(THINGSPEAK_URL, params=payload, timeout=10)
            if r.status_code == 200 and r.text.strip().isdigit():
                print(f"✅ Data sent to ThingSpeak (Entry ID: {r.text.strip()})")
                return True
            else:
    ...
    def save_thingspeak_backup(self, timestamp, data):
        ...
        header_needed = not os.path.exists(THINGSPEAK_BACKUP_FILE)
        try:
            with open(THINGSPEAK_BACKUP_FILE, "a", newline="") as f:
                writer = csv.writer(f)
                if header_needed:
                    writer.writerow(["timestamp", "temperature", "ph", "ec", "nitrogen", "phosphorus", "turbidity", "quality_score", "quality_status"])
                writer.writerow([
                    timestamp,
            ...
            print(f"Error saving ThingSpeak backup: {e}")

    def flush_thingspeak_backup(self):
        """Attempt to resend backed-up readings to ThingSpeak (memory-efficient)."""
        if not os.path.exists(THINGSPEAK_BACKUP_FILE):
            return
        
        rows_to_keep = []
        try:
            with open(THINGSPEAK_BACKUP_FILE, "r", newline="") as f:
                for line in f:
                    # Skip header line
                    if line.startswith("timestamp"):
                        continue
                    # Parse CSV line manually for memory efficiency
                    parts = line.strip().split(',')
                    if len(parts) >= 9:  # Ensure valid data row
                        try:
                            row_dict = {
                                'timestamp': parts[0],
                                'temperature': parts[1],
                                ...
                                'phosphorus': parts[5],
                                'turbidity': parts[6],
                                'quality_score': parts[7],
                                'quality_status': parts[8]
                            }
        ...
        if not rows_to_keep:
            try:
                os.remove(THINGSPEAK_BACKUP_FILE)
            except OSError:
                pass
            return
        ...
        for row in rows_to_keep:
            data = {k: (float(v) if v and k != 'quality_status' else v) for k, v in row.items() if k != 'timestamp'}
            success = self.send_to_thingspeak(data)
            if not success:
                still_failed.append(row)
            else:
                print(f"  Flushed backup: {row['timestamp']}")
            time.sleep(2)  # Respect ThingSpeak rate limit (15 sec min, 2 sec buffer)

        # Write back only failed entries
        if still_failed:
            try:
                with open(THINGSPEAK_BACKUP_FILE, "w", newline="") as f:
                    f.write("timestamp,temperature,ph,ec,nitrogen,phosphorus,turbidity,quality_score,quality_status\n")
                    for row in still_failed:
                        f.write(f"{row['timestamp']},{row['temperature']},{row['ph']},{row['ec']},{row['nitrogen']},{row['phosphorus']},{row['turbidity']},{row['quality_score']},{row['quality_status']}\n")
            except Exception as e:
                print(f"Error writing backup file: {e}")
        else:
            try:
                os.remove(THINGSPEAK_BACKUP_FILE)
                print("  ✅ All backup data sent to ThingSpeak successfully")
            except OSError:
                pass
    ...
    def thingspeak_loop(self):
        ...
        while self.state['running']:
            try:
                if time.time() - self.state['thingspeak']['last_sent_time'] >= THINGSPEAK_SEND_INTERVAL:
                    ...
                    data = {
                        ...
                        'quality_status': self.state.get('last_status_full', {}).get('overall', 'N/A')
                    }
    ...
    def monitor_loop(self):
        while self.state['running']:
            try:
                ...
                # Handle sustained critical state
                if status['overall'] == 'CRITICAL':
                    ...
                    if self.state['critical']['sustained']:
                        ...
                        # Reset critical state if no longer critical
                        if self.state['critical']['sustained']:
...
def main():
    print("Smart Fish Pond Monitoring System v4.0 (ThingSpeak Integration)")
    try:
