...
def distance():
    ...
    # Don't control buzzer from this endpoint - let auto mode handle it
    return jsonify({
        "distance": dist,
        "label": label,
        "status": status,
...
def buzzer_on():
    ...
    auto_control_active = False
    time.sleep(0.3)  # Let auto mode thread exit
...
def buzzer_stop():
    ...
    buzzer_auto_mode = False
    auto_control_active = False
    time.sleep(0.3)  # Let auto mode thread exit properly
